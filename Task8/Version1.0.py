...
def faceRecognition_improve(file):
    ...
    for (x, y, w, h) in faces:
        ...
        roi_gray = img_gray[y:y - int((y + h) * 0.5), x:x + w]
        roi_color = img_rgb[y:y - int((y + h) * 0.5), x:x + w]

        eyes = eye_cascade.detectMultiScale(roi_gray, scaleFactor=1.09, minNeighbors=8)
...
def changePixel(file):
    ...
    img_gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
    print(img_bgr, img_rgb, img_gray)



changePixel("3_people.jpg")